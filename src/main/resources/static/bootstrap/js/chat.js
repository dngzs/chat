...
function userLogin() {
    ...
    if (userNick) {
        ...
        if (window.WebSocket) {
            window.socket = new WebSocket("ws://localhost:8899/websocket");
...
function broadcastInvake(data) {
    ...
    var html = '<div class="title">'+nick+'&nbsp;('+uid+') &nbsp;'+TimeObjectUtil.longMsTimeConvertToDateTime(time)+'</div><div class="item">'+mess+'</div>';
    $("#content").append(html);
    $('#content').scrollTop($('#content')[0].scrollHeight);
